...
class MeshDrawer {
	...
	constructor() {
		...
		this.specularIntensityLoc = gl.getUniformLocation(this.prog, 'specularIntensity'); // New
		this.shininessLoc = gl.getUniformLocation(this.prog, 'shininess');                 // New
	...
	draw(trans) {
		...
		gl.uniform1f(this.specularIntensityLoc, this.specularIntensity); // Set specular intensity
        gl.uniform1f(this.shininessLoc, this.shininess);                 // Set shininess
	...
	setTexture(img) {
		...
		if (isPowerOf2(img.width) && isPowerOf2(img.height)) {
			...
			// Implement handling for NPOT textures
			console.log("Handling NPOT texture");
...
void main() {
    ...
    vec3 viewDir = normalize(vec3(0.0, 0.0, 1.0)); // Assume camera at origin
    ...
    vec3 diffuseLight = diffuseFactor * vec3(1.0, 1.0, 1.0); // White light source
    ...
    vec3 specularLight = specularIntensity * specFactor * vec3(1.0, 1.0, 1.0);
    ...
    if (enableLighting) {
        gl_FragColor = texColor * vec4(lighting, 1.0); // Apply lighting
    } else if (showTex) {
...
function SetSpecularLightIntensity(slider) {
    const newValue = parseFloat(slider.value);
    meshDrawer.specularIntensity = newValue;
	DrawScene();
}


function SetShininess(slider) {
    meshDrawer.shininess = parseFloat(slider.value);
    DrawScene(); // Redraw the scene with the new value
}
